...
export default class ColorPicker extends React.Component {
    ...
    componentDidMount() {
        ...
        const isConicGradientSupported = getComputedStyle(this.paletteRef.current).backgroundImage.includes('conic');

        if (!isConicGradientSupported) {
            fillColorWheel(this.paletteRef.current.firstElementChild, this.elRef.current.offsetWidth || 280);
        }
    ...
    onScroll = ev => {
        if (this.state.isPressed || !this.state.isKnobIn) return;
    ...
    onKeyDown = ev => {
        if (this.props.disabled || this.state.isPressed || !this.state.isKnobIn) return;
    ...
    rotateToMouse = ev => {
        if (this.state.isPressed || !this.state.isKnobIn || ev.target !== this.rotatorRef.current) return;
    ...
    render() {
        ...
        const { isDragging, isPressed, isPaletteIn, isKnobIn, isRippling, ssrHue } = this.state;
        ...
        return (
            ...
            >
                <div ref={this.paletteRef} className={`rcp__palette ${isPaletteIn ? 'in' : 'out'}`}>
                    <canvas />
                </div>
                ...
                >
                    <div className={`rcp__knob ${isKnobIn ? 'in' : 'out'}`} onTransitionEnd={this.hidePalette} />
                </div>
